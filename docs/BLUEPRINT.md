...
7. Build the v1 screens: bindings CRUD form + rule/parameter browser.
   **← current step** (a read-only bindings browser exists as a wiring
   smoke test, but create/edit/delete forms and the rule/parameter browser
   are not built yet).
...
errors, production bundle builds. Not yet done: running `npm run dev` and
actually signing in / exercising the browser screen against the live API
from an interactive session (needs a human at a browser); no Amplify
Hosting deployment yet (§12.1 step 8).
